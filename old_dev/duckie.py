...
def line_intersection(line1, line2):
    """
    Находит точку пересечения двух прямых, заданных двумя точками каждая.

    :param p1: Точка (x1, y1) первой прямой.
    :param p2: Точка (x2, y2) первой прямой.
    :param p3: Точка (x3, y3) второй прямой.
    :param p4: Точка (x4, y4) второй прямой.
    :return: Координаты точки пересечения (x, y) или None, если прямые параллельны.
    """
...
# Параметры робота
dt = 0.1  # шаг времени
v_max = 0.5  # максимальная скорость
...
# Начальное состояние
state = np.array([[0.0],  # Отклонение от центра
                  [0.0]])  # Угол к центру дороги


while not done:
    angle = moving(obs, step)
    angle_test = moving_test(obs)
    step+=1
    center_offset, angle_to_center = moving_test(obs)
    state[0, 0] = center_offset
    state[1, 0] = angle_to_center
    u = -K @ state
    with open('output_u.txt', 'a+') as f:
       f.write(f"{step} u {u} state {state} K {K}\n")
    omega = np.clip(u[0, 0], -omega_max, omega_max)
    action = [v_max, omega]
    with open('output_action.txt', 'a+') as f:
       f.write(f"{step} v_max {v_max} omega {omega}\n")
